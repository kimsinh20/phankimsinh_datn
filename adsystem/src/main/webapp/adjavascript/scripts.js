...
let job_expiration_date_valid  = document.getElementById("job_expiration_date");
if(job_expiration_date_valid!=null) {
	job_expiration_date_valid.min = new Date().toISOString().split("T")[0];
}
...
fetch('http://localhost:8080/adv/api/location/province')
	...
	.then(data => {
		...
		if (document.getElementById('provinces') != null) {
			provinces.map(value => {
				const provinceData = JSON.stringify(value);

				// Tạo một tùy chọn và đặt thuộc tính `data-province` với thông tin `province`.
				const option = document.createElement('option');
				option.setAttribute('data-province', provinceData);
				option.textContent = value.full_name;
				option.value = value.code

				// Thêm tùy chọn vào phần tử select có id là 'provinces'.
	...
	fetch(`http://localhost:8080/adv/api/location/district?code=${provincesID}`)
		...
		.then(data => {
			...
			if (districts !== undefined) {
				districts.map(value => {
					// Tạo một đối tượng JSON từ đối tượng `district`.
					const districtData = JSON.stringify(value);

					// Tạo một tùy chọn và đặt thuộc tính `data-district` với thông tin `district`.
					const option = document.createElement('option');
					option.setAttribute('data-district', districtData);
					option.textContent = value.full_name;
					option.value = value.code

					// Thêm tùy chọn vào phần tử select có id là 'districts'.
...
function fetchWards(districtsID) {
	fetch(`http://localhost:8080/adv/api/location/wards?code=${districtsID}`)
		...
		.then(data => {
			...
			if (wards !== undefined) {
				wards.map(value => {
					// Tạo một đối tượng JSON từ đối tượng `ward`.
					const wardData = JSON.stringify(value);

					// Tạo một tùy chọn và đặt thuộc tính `data-ward` với thông tin `ward`.
					const option = document.createElement('option');
					option.setAttribute('data-ward', wardData);
					option.textContent = value.full_name;
					option.value = value.code

					// Thêm tùy chọn vào phần tử select có id là 'wards'.
...
function addAddress() {
	...
	const wardsObj = JSON.parse(document.getElementById("wards").options[document.getElementById("wards").selectedIndex].getAttribute('data-ward'));
	const districtsObj = JSON.parse(document.getElementById("districts").options[document.getElementById("districts").selectedIndex].getAttribute('data-district'));
	const provincesObj = JSON.parse(document.getElementById("provinces").options[document.getElementById("provinces").selectedIndex].getAttribute('data-province'));
...
function fetchJobView(id) {
	fetch(`http://localhost:8080/adv/api/job/view?id=${id}`)
		...
		.then(data => {
			...
			let job_skills = document.getElementById('job_skill');
			const job_skill_id = job.job_skills.split(',');
			for (let option of job_skills.options) {
				// Kiểm tra xem giá trị của option có trong mảng selectedValues không
				if (job_skill_id.includes(option.value)) {
					// Nếu có, gắn thuộc tính selected cho option
					option.selected = true;
				}
			}
			let elementsToRemove = document.querySelectorAll('.mult-select-tag');

			// Duyệt qua danh sách các phần tử và xóa chúng
...
function fetchJobViewbyEmployer(job_id,com_id) {
	try {
		async function loadData() {
			await getCareerByCompanyV2(com_id);
			await fetchJobViewV2(job_id);
...
const fetchCareer = async (companyId) => {
	try {
		...
		document.getElementById('job_career').innerHTML += `<option value='0'>Khác</option>`;
	} catch (error) {
		console.error('Lỗi khi gửi API:', error);
	}
}
...
function getCareerByCompanyV2(company_id) {
	fetchCareer(company_id);
	fetchLocation(company_id);
}
...
function fetchUpdateStatusApp(app_id, status) {
	fetch(`http://localhost:8080/adv/api/app/status?app_id=${app_id}&app_status=${status}`)
		.then(response => {
			if (response.status === 200) {
				Toastify({
					text: "Trạng thái ứng tuyển đã được cập nhật thành công",
					duration: 3000,
					destination: "http://localhost:8080/adv/app/list",
					newWindow: true,
					close: true,
					gravity: "bottom", // `top` or `bottom`
					position: "right", // `left`, `center` or `right`
				...
				console.log('Trạng thái ứng tuyển đã được cập nhật thành công.');
			} else {
				// Yêu cầu thất bại
				Toastify({
					text: "Thất bại",
				...
				console.error('Lỗi khi cập nhật trạng thái ứng tuyển:', data.message);
			}
		})
		.catch(error => {
			console.error('L?i khi g?i API:', error);
