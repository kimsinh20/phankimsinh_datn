...
//		System.out.println(saveKey);
		...
		byte pageSize = 5;
		Quartet<UserObject, Integer, Byte,USER_TYPE> infos = new Quartet<>(similar, pageSize*(page-1), pageSize,USER_TYPE.ADMIN);
...
//		out.append("<option value=\"1\">Ứng viên</option>");
//		out.append("<option value=\"2\">Nhà tuyển dụng</option>");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (name != null && !name.equalsIgnoreCase("") && jsoft.library.Utilities_text.checkValidPass(pass, comfirmPass)
			...
			boolean result = uc.addUser(newUser,USER_TYPE.ADMIN);
			// return connect
			uc.releaseConnection();
			if (result) {
