...
public class AppLibrary {
	public static String viewApp(ArrayList<ApplicationsObject> items,Short total,int page,UserObject user) {
		...
		items.forEach(item -> {
			...
			tmp.append("<td class=\"text-center\"><a href=\"/adv/apply/dr?id=" + item.getApplications_id() + "&t&r&page=" + page
	...
	public static String viewApp2(ArrayList<ApplicationsObject> items,Short total,int page,UserObject user) {
		StringBuffer tmp = new StringBuffer();
		...
		items.forEach(item -> {
			...
			tmp.append("<th scope=\"row\">"+(items.indexOf(item) + 1)+"</th>");
			tmp.append("<td>"+item.getApplications_created_date()+"</td>");
			tmp.append("<td>"+item.getJob().getJob_title()+"<br/><a href=\"/adv/employer/jobview?id="+item.getJob().getJob_id()+"\">Xem chi tiết</a>"+"</td>");
			tmp.append("<td>"+item.getUser().getUser_fullname()+"<br/><a href=\"/adv/client/profiles?id="+item.getUser().getUser_id()+"&view&page=1\">Xem hồ sơ</a>"+"</td>");
...
//		out.append(" <li class=\"page-item\"><a class=\"page-link\" href=\"/adv/user/list?page="+totalPage+"\">"+totalPage+"</a></li>");
	...
	private static StringBuilder ViewDellApp(ApplicationsObject item, int page) {
		...
		if (item.isApplications_delete()) {
			tmp.append("Bạn sẽ vĩnh viễn xóa đơn ứng tuyển <b>").append(" (" + item.getApplications_id() + ")</b>");
			tmp.append("<p>Tài khoản không thể phục hồi được nữa!!!</p>");
		} else {
			tmp.append("Bạn có chắc chắn xóa đơn ứng tuyển <b>").append(" (" + item.getApplications_id() + ")</b>");
		...
		if (item.isApplications_delete()) {
			tmp.append("<a href=\"/adv/apply/dr?id=" + item.getApplications_id() + "&page="
					+ page + "\" class=\"btn btn-danger px-5 py-2\">Xóa vĩnh viễn</a>");
		} else {
			tmp.append("<a href=\"/adv/apply/dr?id=" + item.getApplications_id() + "&t"
