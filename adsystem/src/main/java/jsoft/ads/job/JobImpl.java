...
public class JobImpl extends BasicImpl implements Job {
	...
	public boolean editJob(JobObject item, JOB_EDIT_TYPE et) {
		...
		case GENERAL:
			...
			sql.append("job_company_id =? , job_last_modified =?, job_author_id = ?, job_degree = ?,job_career_id =? ,job_location=?,job_skills=? ");
			break;
		case TRASH:
		...
		//	PreparedStatement pre2 = this.con.prepareStatement(sql2.toString());
			...
			case GENERAL:
				...
				pre.setString(18, item.getJob_skills());
				pre.setInt(19, item.getJob_id());
				break;
			case TRASH:
...
//            System.out.println(sql);
	...
	public ArrayList<ResultSet> getJob(short id, UserObject userLogined) {
		...
		sql.append("WHERE applications_job_id = "+id+" AND applications_delete=0");
		System.out.println(sql);
	...
	private String createConditions(JobObject similar) {
		...
		if (similar != null) {
			...
			if(similar.getCompany()!=null) {
				if (similar.getCompany().getCompany_id() > 0) {
					conds.append(" AND (job_company_id= "+similar.getCompany().getCompany_id()+") ");
				} 
			}
			
			if (similar.getJob_author_id()> 0) {
				conds.append(" AND (job_author_id= "+similar.getJob_author_id()+") ");
			} 
