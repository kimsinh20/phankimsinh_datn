...
public class FieldLibrary {
	public static String viewField(ArrayList<FieldObject> items,Short total,HashMap<Integer, String> author,int page,UserObject user) {
		...
		tmp.append("<th scope=\"col\">Tên lĩnh vực</th>");
		...
		items.forEach(item -> {
			...
			tmp.append("<td>"+item.getField_created_date()+"</td>");
			tmp.append("<td>"+item.getField_name()+"</td>");
			tmp.append("<td>"+author.get(item.getField_author_id())+"</td>");
			tmp.append("<td>"+(jsoft.library.Utilities_text.shortenText(item.getField_notes(), 10))+"</td>");
			if(item.isField_delete()) {
			tmp.append("<td class=\"text-center\"><a href=\"/adv/field/dr?id=" + item.getField_id() + "&t&r&page=" + page
					+ "\" class=\"btn btn-primary btn-sm\"><i class=\"fas fa-trash-restore-alt\"></i></a></td>");
			}
			tmp.append("<td><a href=\"/adv/field/edit?page="+page+"&id="+item.getField_id()+"&view\" class=\"btn btn-primary btn-sm\"><i class=\"bi bi-eye-fill\"></i></a></td>");
			tmp.append("<td><a href=\"/adv/field/edit?page="+page+"&id=" + item.getField_id()
				+ "\" class=\"btn btn-secondary btn-sm\"><i class=\"bi bi-pencil-square\"></i></a></td>");
			tmp.append("<td class=\"text-center\"><a href=\"#\" class=\"btn btn-warning btn-sm\" data-bs-toggle=\"modal\" data-bs-target=\"#delField"
				+ item.getField_id() + "\"><i class=\"bi bi-archive\"></i></a></td>");
			tmp.append(FieldLibrary.ViewDellField(item, page));
			tmp.append("<th scope=\"row\">"+item.getField_id()+"</th>");
			tmp.append("</tr>");			
		});
	   } else {
...
//		System.out.println(total);
		...
		out.append("<a class=\"page-link\" href=\"/adv/field/list?page="+(page > 1 ? page - 1 : 1)+urlkey+"\" tabindex=\"-1\" aria-disabled=\"true\"><span aria-hidden=\"true\">&laquo;</span></a></a>");
		out.append("</li>");
		if(page>=4) {
		out.append("<li class=\"page-item\">");
		out.append("<a class=\"page-link\" href=\"/adv/field/list?page=1"+urlkey+"\" tabindex=\"-1\" aria-disabled=\"true\"><span aria-hidden=\"true\">1</span></a></a>");
		out.append("</li>");
		}
		//left current
		String leftCurrent = "";
		...
		for(int i=page-1;i>0;i--) {
			leftCurrent = "<li class=\"page-item\"><a class=\"page-link\" href=\"/adv/field/list?page="+i+urlkey+"\">"+i+"</a></li>"+leftCurrent;
		...
		out.append("<a class=\"page-link\" href=\"/adv/field/list?page="+page+urlkey+"\">"+page+"</a>");
		out.append("</li>");
		
		String rightCurrent ="";
		 count = 0;
		for(int i=page+1;i<=totalPage;i++) {
			rightCurrent+="<li class=\"page-item\"><a class=\"page-link\" href=\"/adv/field/list?page="+i+urlkey+"\">"+i+"</a></li>";
			if(++count>=2) {
				break;
			}
		}
...
//		out.append(" <li class=\"page-item\"><a class=\"page-link\" href=\"/adv/user/list?page="+totalPage+"\">"+totalPage+"</a></li>");
		if(page<totalPage-2) {
		out.append("<li class=\"page-item\" >");
		out.append("<a class=\"page-link\"  href=\"/adv/field/list?page="+totalPage+urlkey+"\"><span aria-hidden=\"true\">"+totalPage+"</span></a></a>");
		out.append("</li>");
		}
		out.append("<li class=\"page-item "+isNext+"\" >");
		out.append("<a class=\"page-link\"  href=\"/adv/field/list?page="+(page < totalPage ? page + 1 : totalPage)+urlkey+"\"><span aria-hidden=\"true\">&raquo;</span></a></a>");
		out.append("</li>");
		out.append("</ul>");
	...
	private static StringBuilder ViewDellField(FieldObject item, int page) {
		...
		tmp.append("<div class=\"modal modal-fullscreen-sm-down fade\" id=\"delField" + item.getField_id()
		...
		if (item.isField_delete()) {
			tmp.append("Bạn sẽ vĩnh viễn xóa lĩnh vực <b>").append(" (" + item.getField_name() + ")</b>");
			tmp.append("<p>Tài khoản không thể phục hồi được nữa!!!</p>");
		} else {
			tmp.append("Bạn có chắc chắn xóa lĩnh vực <b>").append(" (" + item.getField_name() + ")</b>");
		...
		if (item.isField_delete()) {
			tmp.append("<a href=\"/adv/field/dr?id=" + item.getField_id() + "&page="
					+ page + "\" class=\"btn btn-danger px-5 py-2\">Xóa vĩnh viễn</a>");
		} else {
			tmp.append("<a href=\"/adv/field/dr?id=" + item.getField_id() + "&t"
