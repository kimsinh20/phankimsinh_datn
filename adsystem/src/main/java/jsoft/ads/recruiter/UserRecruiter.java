 ...
 */
@WebServlet("/recruiter/list")
public class UserRecruiter extends HttpServlet {
...
//		System.out.println(saveKey);
		...
		byte pageSize = 5;
		Quartet<UserObject, Integer, Byte,USER_TYPE> infos = new Quartet<>(similar, pageSize*(page-1), pageSize,USER_TYPE.RECRUITER);
		...
		String pos = (trash==null)?"/header?pos=urrecruiter":"/header?pos=urtrash";
		...
		out.append("<div class=\"col\">");
		out.append("<label for=\"address\" class=\"form-label\"><i class=\"fas fa-user-tag\"></i>Công ty</label>");
		out.append("<select class=\"form-select\" id=\"usercompany\" name=\"slcCompany\" required>");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		byte permis = 2;
		int companyid = jsoft.library.Utilities.getIntParam(request, "slcCompany");
		...
		if (name != null && !name.equalsIgnoreCase("") && jsoft.library.Utilities_text.checkValidPass(pass, comfirmPass)
			...
			RecruiterObject newUser = new RecruiterObject();
			...
			boolean result = uc.addUser(newUser,USER_TYPE.RECRUITER);
			// return connect
			uc.releaseConnection();
			if (result) {
				response.sendRedirect("/adv/recruiter/list");
			} else {
				response.sendRedirect("/adv/recruiter/list?err=add");
			}
			...
			response.sendRedirect("/adv/recruiter/list?page=1"+key_url);
		}
	}
