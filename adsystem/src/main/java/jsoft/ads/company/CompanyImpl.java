...
public class CompanyImpl extends BasicImpl implements Company {
	...
	public boolean addCompany(CompanyObject item) {
		...
		sql.append("`company_location`,`company_banner`,`company_field_id`,`company_tax_code`) ");
		sql.append("VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);");
		sql.append("");
		...
		try {
			...
			pre.setString(21, item.getCompany_tax_code());
	...
	public boolean editCompany(CompanyObject item, COMPANY_EDIT_TYPE et) {
		...
		case GENERAL:
			String banner = "",logo="";
			if(!item.getCompany_banner().equalsIgnoreCase("")) {
				banner="`company_banner`=?,";
			}
			if(!item.getCompany_logo().equalsIgnoreCase("")) {
				logo="`company_logo`=?,";
			}
			sql.append("`company_name`=?, `company_summary`=?, `company_mobilephone`=?, ");
			sql.append("`company_homephone`=?, `company_officephone`=?, ");
			...
			sql.append("`company_field_id`=?, "+logo+" "+banner+"  `company_location`=? ");
			break;
		case TRASH:
		...
		try {
			...
			case GENERAL:
				...
				pre.setString(4, item.getCompany_homephone());
				pre.setString(5, item.getCompany_officephone());
				pre.setString(6, item.getCompany_email());
				pre.setString(7, item.getCompany_about());
				pre.setString(8, item.getCompany_remuneration());
				pre.setInt(9, item.getCompany_nationality());
				pre.setInt(10, item.getCompany_size());
				pre.setInt(11, item.getCompany_work_time());
				pre.setBoolean(12, item.isCompany_isOT());
				pre.setString(13, item.getCompany_website());
				pre.setString(14, item.getCompany_last_modified());
				pre.setInt(15, item.getCompany_field_id());
				if(!item.getCompany_banner().equalsIgnoreCase("") && !item.getCompany_logo().equalsIgnoreCase("")) {
					pre.setString(16, item.getCompany_logo());
					pre.setString(17, item.getCompany_banner());
					pre.setString(18, item.getCompany_location());
					pre.setInt(19, item.getCompany_id());
				} else if(item.getCompany_banner().equalsIgnoreCase("") && !item.getCompany_logo().equalsIgnoreCase("") ) {
					pre.setString(16, item.getCompany_logo());
					pre.setString(17, item.getCompany_location());
					pre.setInt(18, item.getCompany_id());
				} else if(!item.getCompany_banner().equalsIgnoreCase("") && item.getCompany_logo().equalsIgnoreCase("") ) {
					pre.setString(16, item.getCompany_banner());
					pre.setString(17, item.getCompany_location());
					pre.setInt(18, item.getCompany_id());
				} else {
					pre.setString(16, item.getCompany_location());
					pre.setInt(17, item.getCompany_id());
				}
...
//            System.out.println(sql);
	...
	public ArrayList<ResultSet> getCompany(short id, UserObject userLogined) {
		...
		sql.append("SELECT * FROM tbljob where job_delete=0 AND job_company_id = "+id+" ;");
		System.out.println(sql);
	...
	public ArrayList<ResultSet> getCompanies(Quartet<CompanyObject, Integer, Byte, UserObject> infos, Pair<COMPANY_SOFT, ORDER> so) {
		...
		// bieu do 
		sql.append("SELECT company_id,company_name,company_subscribe FROM tblcompany group by company_id,company_name,company_subscribe  order by company_subscribe desc limit 0,5;");
