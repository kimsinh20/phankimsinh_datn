...
public class CompanyLibrary {
	public static String viewCompany(ArrayList<CompanyObject> items,Short total,HashMap<Integer, String> author,int page,UserObject user) {
		...
		tmp.append("<th scope=\"col\">Logo</th>");
		...
		items.forEach(item -> {
			...
			tmp.append("<th scope=\"row\"><img src=\"" + item.getCompany_logo() + "\" alt=\"company_logo\" class=\"rounded-circle logo_img\"></th>");
...
//		out.append(" <li class=\"page-item\"><a class=\"page-link\" href=\"/adv/user/list?page="+totalPage+"\">"+totalPage+"</a></li>");
	...
	public static StringBuilder createdChart(ArrayList<CompanyObject> items) {
		StringBuilder data = new StringBuilder();
		StringBuilder company = new StringBuilder();
		items.forEach(item -> {
			data.append(item.getCompany_subribe());
			company.append("'" + Utilities.decode(item.getCompany_name())).append("'");
		...
		tmp.append("<div class=\"card\">");
		tmp.append("<div class=\"card-body\">");
		tmp.append("<h5 class=\"card-title\">Top 5 công ty có lượt theo dõi nhiều nhất</h5>");
		tmp.append("");
		...
		tmp.append("},");
		tmp.append("xaxis: {");
		tmp.append("categories: [" + company.toString() + "],");
		tmp.append("}, ");
		tmp.append("yaxis: {");
