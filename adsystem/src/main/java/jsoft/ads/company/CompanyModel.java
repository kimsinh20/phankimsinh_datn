...
//	----------------------------------------
	...
	public Quartet<CompanyObject, HashMap<Integer, String>,ArrayList<FieldObject>,ArrayList<JobObject>> getCompanyObject(short id, UserObject userLogined) {
		...
		if(rs!=null) {
			try {
				if(rs.next()) {
					...
					System.out.println(jsoft.library.Utilities.decode(rs.getString("company_location")));
		...
		if(rs!=null) {
			try {
				while(rs.next()) {
				    ...
				    CompanyObject c= new CompanyObject();
				    c.setCompany_id(rs.getInt("job_company_id"));
				    Job.setCompany(c);
		...
		return new Quartet<>(company, author, fields,jobs);
	}
	public Quintet<ArrayList<CompanyObject>, Short, HashMap<Integer,String>,ArrayList<FieldObject>,ArrayList<CompanyObject>> getCompanyObjects
	(Quartet<CompanyObject, Integer, Byte,UserObject> infos, Pair<COMPANY_SOFT, ORDER> so) {
		...
		// bieu do
		ArrayList<CompanyObject> datas = new ArrayList<>();
		CompanyObject data = null;
		 rs = res.get(3);
		if(rs!=null) {
			try {
				while(rs.next()) {
					data = new CompanyObject();
					data.setCompany_id(rs.getShort("company_id"));
					data.setCompany_name(rs.getString("company_name"));
					data.setCompany_subribe(rs.getInt("company_subscribe"));
					datas.add(data);
				}
				rs.close();
			} catch (SQLException e) {
		...
		return new Quintet<>(companies,total,author,fields,datas);
	}
}
