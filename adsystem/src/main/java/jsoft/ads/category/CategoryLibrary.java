...
public class CategoryLibrary {
	...
	public static String viewSectionOptions(ArrayList<SectionObject> sections) {
		StringBuilder tmp = new StringBuilder();
...
//		out.append(" <li class=\"page-item\"><a class=\"page-link\" href=\"/adv/user/list?page="+totalPage+"\">"+totalPage+"</a></li>");
	...
	private static StringBuilder ViewDellCategory(CategoryObject item, int page) {
		...
		if (item.isCategory_delete()) {
			tmp.append("Bạn sẽ vĩnh viễn xóa thể loại <b>").append(" (" + item.getCategory_name() + ")</b>");
			tmp.append("<p>Tài khoản không thể phục hồi được nữa!!!</p>");
		} else {
			tmp.append("Bạn có chắc chắn xóa thể loại <b>").append(" (" + item.getCategory_name() + ")</b>");
