...
public class JobList extends HttpServlet {
	...
	protected void view(HttpServletRequest request, HttpServletResponse response, RecruiterObject user,int page)
		...
		similar.setJob_author_id(user.getUser_id());
		...
		String pos = (trash==null)?"/employer/header?pos=jblist":"/employer/header?pos=jbtrash";
		...
		if (checkValidString(job_title) && checkValidString(job_expiration_date) 
			...
			if(id>0) {
				...
				if (resultAdd) {
					...
					response.sendRedirect("/adv/employer/job?err=add");
				}
			}
		} else {
