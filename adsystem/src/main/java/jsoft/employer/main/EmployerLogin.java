...
public class EmployerLogin extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		out.append("<link href=\"/adv/adcss/user.css\" type=\"text/css\" rel=\"stylesheet\">");
		...
		out.append("<script language=\"javascript\" src=\"/adv/adjavascript/auth.js\"></script>");
		...
		out.append("<span class=\"d-none d-lg-block fs-3 text-dark fw-bold\" id=\"brand\">ADMIN JOBNOW</span>");
		out.append("</a>");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (username != null && userpass != null) {
			...
			if (!username.equalsIgnoreCase("") && !userpass.equalsIgnoreCase("")) {
				...
				// Thực hiện đăng nhập
				RecruiterObject user = uc.getEmployerObject(username, userpass);
				...
				if (user != null) {
						if(user.getUser_permission()==2) {
							session.setAttribute("employerLogined", user);
							response.sendRedirect("/adv/employer?success");
					} else {
						
					}
				} else {
					response.sendRedirect("/adv/employer/login?err=loginfail");
				}

			} else {
				response.sendRedirect("/adv/employer/login?err=value");
			}

		} else {
			response.sendRedirect("/adv/employer/login?err=param");
