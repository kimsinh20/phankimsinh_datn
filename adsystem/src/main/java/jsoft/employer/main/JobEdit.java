...
public class JobEdit extends HttpServlet {
	...
	protected void view(HttpServletRequest request, HttpServletResponse response, RecruiterObject user,int page)
		...
		CompanyObject cm = new CompanyObject();
		cm.setCompany_id(user.getCompany_id());
		similar.setCompany(cm);
		similar.setJob_title(saveKey);

		String title = "Cập nhật tin tuyển dụng";
		...
		Quartet<JobObject, HashMap<Integer, String>,ArrayList<SkillObject>,ArrayList<UserObject>> data = jc.getJobObject(id, user);
		
		JobObject job = data.getValue0();
		short company_id =(short) job.getCompany().getCompany_id();
		ArrayList<CareerObject> careers = jc.getCareerByCompany(company_id);
		// trả về kết nối
		jc.releaseConnection();

		
		String pos = "/employer/header?pos=jblist";
		...
		out.append("<h2 class=\"text-center\">Thông tin về tin tuyển dụng</h2>");
		...
		out.append("<input type=\"text\" class='form-control' value=\""+job.getJob_title()+"\" name=\"job_title\" id=\"job_title\" placeholder=\"...\" oninput=\"this.className = 'form-control'\">");
		out.append("</div>");
		...
		out.append(JobLibrary.viewCareerOptions(careers));
		out.append("</select>");
		out.append("</div>");
		out.append("</div>");//end row
        ...
        out.append(JobLibrary.viewSkillOptions(data.getValue2()));
		out.append("</select>");
		out.append("</div>");
		out.append("</div>");//end row
       ...
       // tab 4
		...
		out.append("<input type=\"hidden\" value=\""+user.getCompany_id()+"\" id=\"job_company\" name=\"job_company\">");
		out.append("<input type=\"hidden\" value=\""+job.getJob_id()+"\" id=\"idForPost\" name=\"idForPost\">");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (checkValidString(job_title) && checkValidString(job_expiration_date) 
			...
			if (resultEdit) {
				response.sendRedirect("/adv/employer/job");
			} else {
				response.sendRedirect("/adv/employer/job?err=edit");
			}
		} else {
			String key = request.getParameter("keyword");
			String encodedKeyword = URLEncoder.encode(key, "UTF-8");
			String key_url = (key!=null && !key.equalsIgnoreCase(""))?("&key="+encodedKeyword):"";
			response.sendRedirect("/adv/employer/job?page=1"+key_url);
		}
	}
